...
def on_mouse_press(x, y, button, modifiers):
    ...
    if button == pyglet.window.mouse.LEFT:
        ...
        if (x >= PlayAI.x and x <= PlayAI.x + PlayAI.width) and (y >= PlayAI.y and y <= PlayAI.y + PlayAI.height):
            ...
            HIGHSCORE = 0
            agent = Agent()
            agent.targetModel.load_model()
            agent.targetModel.eval()
            games = InitGames(window, WINDOWSIZE, DENSITY, 1)
            pyglet.clock.unschedule(update)
            pyglet.clock.schedule_interval(update, SPEED)
...
def playAI():
    global HIGHSCORE
    game = games[0]
    move = agent.get_action_testing(game.get_simple_state())
...
def train():
    ...
    if len(plot_mean_scores) < 201:
        
        for game in games:
            state_old = game.get_simple_state()
            ...
            state_new = game.get_simple_state()
        ...
        if total_steps > 200: 
            ...
            # if len(plot_mean_scores) == 0:
            #     plot_mean_scores.append(0)

            mean_score = sum(last_hundred) / len(last_hundred)
